...
class StockAssessor
  ...
  def initialize
    ...
    @priceRateGeneralEvolveHandler = nil # craete handler.

    # @priceRateGeneralEvolveHandler.stockHistoryReporter=@stockHistoryReporter # 设置历史报告器
    # @priceRateGeneralEvolveHandler.eventChart=@eventChart # Set the vent chart object
  end 
    ...
    def getTransactionList(requestId)
      if @priceRateGeneralEvolveHandler.nil? # the handler does not exist
        @priceRateGeneralEvolveHandler = PriceRateGeneralEvolveHandler.new # craete handler.
        @priceRateGeneralEvolveHandler.stockHistoryReporter=@stockHistoryReporter # 设置历史报告器
        @priceRateGeneralEvolveHandler.eventChart=@eventChart # Set the vent chart object
      end # if @priceRateGeneralEvolveHandler.nil? # the handler does not exist
      
      resultObject = @priceRateGeneralEvolveHandler.getTransactionList( requestId) # Get transaction list.
    ...
    def assessStockPriceGeneralEvolve(stockCodeOrAlias, moneyInput, dayAmount, ruleList, requestId)
      ...
      resultObject = @priceRateGeneralEvolveHandler.assessStockPriceGeneralEvolve(stockCodeOrAlias, moneyInput, dayAmount, ruleList, requestId) # assess using the handler object.
    ...
    def getClosePrice(stockCode) 
      ...
      if (dateDataList.length>=1) # 数据长度足够
        begin #开始计算，可能会有价格过贵，买不起的情况
          ...
          resultObject[FieldCode::MaxDateAmount] = (dateDataList.length - 1) # Send the max day amount.
