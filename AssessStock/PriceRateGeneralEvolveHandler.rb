...
class PriceRateGeneralEvolveHandler
  ...
  def normalizePriceCeil(sellprice) 
    ...
    def assessStockPriceGeneralEvolveRuleList(stockCodeOrAlias, moneyInput, dayAmount, ruleList,  requestId)
      ruleObjectList, sellRuleObjectList = @stockRuleFactory.parseRuleList(ruleList) # parse rule list.
        ...
        if (dataLengthEnough) # 数据长度足够
          ...
          begin # 开始计算，可能会有价格过贵，买不起的情况
            while (allowedBuyInDateIndex>=0) do #还有买入机会
              allowedBuyInDateIndex, transactionObject, sellTransactionObject = findRuleListBuySellPairAddTransaction(initialMoney, dateDataList, allowedBuyInDateIndex, ruleObjectList, sellRuleObjectList) # 寻找满足要求的买卖对，并且加入到事务列表中
              ...
              # transactionObject['soFarProfit']=profit # 记录目前为止的收益. Buy type.
              transactionObject[FieldCode::SoFarProfit] = profit # 记录目前为止的收益. Buy type.
            ...
            # puts "#{__LINE__}, transaction amount: #{transactionList.size}" # Debug.
            ...
            resultObject[FieldCode::TransactionAmount] = transactionList.size # add transaftion amount.
            resultObject['stockCodeAlias']=aliasForThisStockCode # add the alias of stock code
            
            if (transactionList.size > 0) # There is at least one transaction
              # puts "#{__LINE__}, last transaction date: #{transactionList.last['date']}" # Debug.
              resultObject[FieldCode::LastTransactionDate] = transactionList.last['date'] # set the last transaction date.
            end # if (transactionList.size > 0) # There is at least one transaction
            
            suggestedBuyInPrice = findBuyInPriceByRules( dateDataList , ruleObjectList, -1) # 计算出 suggested  买入价. By using rule list.
            suggestedSellOutPrice = findSellOutPriceByRules( dateDataList, sellRuleObjectList, -1 ) # 计算出 suggested 卖出价. By using rule list.

            priceMapLastDate=dateDataList[0]['priceMap'] #获取上一天的价格映射
            lastClosePrice = priceMapLastDate['close'] #获取上一天收盘价
            
            suggestedHighRate = suggestedSellOutPrice.to_f / lastClosePrice # Calcluate suggested high rate.
            suggestedLowRate = suggestedBuyInPrice.to_f / lastClosePrice # Calcluate suggested low rate.
            
            resultObject[FieldCode::SuggestedLowRate] = suggestedLowRate # add suggested low rate.
            resultObject[FieldCode::SuggestedHighRate] = suggestedHighRate # add suggested high rate.
          rescue FloatDomainError => e #无限大
            # puts "#{__LINE__}, rule list: #{ruleList}" # Debug.
            puts("rescue, #{e}") #Debug
    ...
    def findSellOutPriceByRules( dateDataList, ruleList, currentCandidateSellDateIndx )
      candidateBuyInPriceList = [] # candidate buy in price list.

      ruleList.each do |currentRule| # check rules one by one
        currentBuyInPrice = currentRule.getSellOutPrice( dateDataList, currentCandidateSellDateIndx ) # Get the sell out price suggested by this rule.
        
        if currentBuyInPrice.nil? # Cannot give buy in price from this rule
        else # Gave valid buy in price
      ...
      result = candidateBuyInPriceList.min # get the min price of sell out.
